...
let rec default_instruction_operation () : instruction_operation = Op_idx (0l)

and default_instruction 
  ?inouts:((inouts:int32 list) = [])
  ?operation:((operation:instruction_operation) = Op_idx (0l))
  () : instruction  = {
  inouts;
  operation;
}
...
let rec default_program 
  ?uuid:((uuid:bytes) = Bytes.create 0)
  ?code:((code:instruction list) = [])
  ?comments:((comments:(int32 * string) list) = [])
  ?parent:((parent:program option) = None)
  () : program  = {
  uuid;
  code;
  comments;
