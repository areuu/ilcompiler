...
let build_test_instanceof_op lvar rvar builder = 
    let result_var = get_new_intermed_temp builder in
    let inst = Program_types.{
        inouts = [lvar; rvar; result_var];
        operation = Test_instance_of;
    } in
    result_var, inst

let build_test_in_op lvar rvar builder = 
    let result_var = get_new_intermed_temp builder in
    let inst = Program_types.{
        inouts = [lvar; rvar; result_var];
        operation = Test_in;
...
let build_continue builder = 
    Program_types.{
        inouts = [];
        operation = Program_types.Loop_continue;
    }
...
let build_begin_if var builder = 
    let begin_if_inst = Program_types.{
        inouts = [var];
        operation = Begin_if Operations_types.{inverted= false}; 
    } in
    begin_if_inst
...
let build_switch_break_op builder = 
    Program_types.{
        inouts = [];
        operation = Program_types.Switch_break;
    }

let build_loop_break_op builder = 
    Program_types.{
        inouts = [];
        operation = Program_types.Loop_break;
    }
...
let build_func_ops func_var arg_names rest_arg_name_opt is_arrow is_async is_generator builder =
    ...
    let proced_ids = List.map temp_func arg_names in  (* proced_ids: var *)
  (*  let temps = List.split proced_ids in *)

    let rest_temp = match rest_arg_name_opt with   (* TODO check *)
        None -> []
        | Some rest_arg_name ->
    ...
    let all_temps = proced_ids @ rest_temp in
  
    let _length = List.length all_temps in 
    let func_parameters: Operations_types.parameters = Operations_types.{  (* for test *)
      count = Int32.of_int(_length); 
      has_rest = false;
    } in
    ...
    let begin_inst_op, end_inst_op = 
        if is_arrow then
            if is_async then
                let begin_func_op : Operations_types.begin_async_arrow_function 
                    = Operations_types.{parameters=Some func_parameters; is_strict=false} in  
                let inst_op = Program_types.Begin_async_arrow_function begin_func_op in
                let end_op = Program_types.End_async_arrow_function in
                inst_op, end_op
            else
                (* Norm arrow *)
                let begin_func_op : Operations_types.begin_arrow_function
                    = Operations_types.{parameters=Some func_parameters; is_strict=false} in
                let inst_op = Program_types.Begin_arrow_function begin_func_op in
                let end_op = Program_types.End_arrow_function in
                inst_op, end_op
        else
            if is_async then
                (* Norm Async*)
                let begin_func_op : Operations_types.begin_async_function
                    = Operations_types.{parameters=Some func_parameters; is_strict=false} in
                let inst_op = Program_types.Begin_async_function begin_func_op in
                let end_op = Program_types.End_async_function in
                inst_op, end_op
            else
                if is_generator then
                    (* Generator*)
                    let begin_func_op : Operations_types.begin_generator_function
                        = Operations_types.{parameters=Some func_parameters; is_strict=false} in
                    let inst_op = Program_types.Begin_generator_function begin_func_op in
                    let end_op = Program_types.End_generator_function in
                    inst_op, end_op
                else
                    let begin_func_op : Operations_types.begin_plain_function
                        = Operations_types.{parameters=Some func_parameters; is_strict=false} in
                    let inst_op = Program_types.Begin_plain_function begin_func_op in
                    let end_op = Program_types.End_plain_function in
                    inst_op, end_op
        in
